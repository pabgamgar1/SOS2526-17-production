...
const BASE_URL_API_V2 = "/api/v2/renewable-energy-consumptions";
...
function loadBackendPGG_v2(app) {
  // Documentación v2
  app.get(BASE_URL_API_V2 + "/docs", (req, res) => {
    const target = "https://documenter.getpostman.com/view/52407239/2sBXigMYvk";
    res.redirect(target);
  });

  // GET colección con filtros y paginación (v2: responde también con metadatos total)
  app.get(BASE_URL_API_V2, (req, res) => {
    ...
    db.find({}, (err, stats) => {
      ...
      // v2: incluye metadatos de paginación en la respuesta
  ...
  // POST recurso específico (no permitido)
  app.post(BASE_URL_API_V2 + "/:country/:year", (req, res) =>
    res.sendStatus(405),
  );
  ...
  // PUT recurso específico
  app.put(BASE_URL_API_V2 + "/:country/:year", (req, res) => {
    const { country, year } = req.params;
    const updatedData = req.body;
  ...
  // DELETE colección
  app.delete(BASE_URL_API_V2, (req, res) => {
    if (req.query.admin !== "true") {
      return res.sendStatus(401);
    }
  ...
  // DELETE recurso específico
  app.delete(BASE_URL_API_V2 + "/:country/:year", (req, res) => {
    const { country, year } = req.params;
...
export { loadBackendPGG, loadBackendPGG_v2 };
